...
keywords:
  - OpenEBS benefits
description: Some key aspects that make OpenEBS different compared to other traditional storage solutions include - built using a micro-services architecture, highly portable to run across any OS/platform, completely intent-driven and declarative, and including a range of storage engines.
---
...
- [High availability - with a Lower Blast Radius](#high-availability)
...
<TwoColumn left="1fr" right="200px">
  <p>
    ...
    With OpenEBS, the data can be written to the OpenEBS layer - if a Replicated Storage 
...
<TwoColumn left="1fr" right="200px">
  <p>
    ...
    worrying about managing the persistent storage needs of stateful applications. 
...
<TwoColumn left="1fr" right="200px">
  <p>
    ...
    kept local to the volume. Losing any node results in the loss of only those volume
    replicas present on that node. As the volume data can be synchronously
    replicated at other nodes, in the event of a node failure, the
