...
These events have two consumers. Within the obs-callhome-stats container, the stats exporter listens on the event bus, collects these events, and makes them available as part of callhome metrics. The [Eventing Aggregator](eventing-aggregator.md) also subscribes to the event bus and records every event in a queryable form, so that the events listed below can be retrieved with the `kubectl mayastor get events` command.
