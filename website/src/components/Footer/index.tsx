import {
  Toolbar, Paper, Typography, Link, Button,
...
const Footer: React.FC = () => {
  ...
  const displayNewsLetter = () => (
    <div>
      ...
      <Button
        ...
        onClick={() => {
          window.open(EXTERNAL_LINKS.SUBSCRIBE_NEWSLETTER, '_blank');
        }}
      >
      ...
      <Typography className={classes.columnListWrapper}>
        {getStarted.map(({ label, href }) => (
          <Link
            ...
            key={label}
            target={
              label === EXTERNAL_LINK_LABELS.GITHUB ? '_blank' : '_parent'
            }
          >
            {label}
          </Link>
  ...
  const displayMobileFooter = () => (
    <Toolbar className={classes.toolbar}>
      <Grid container spacing={3}>
            ...
            <div>
              <Button className={classes.contributeButton}>
                <img
                  ...
                  className={classes.githubMobileIcon}
        ...
        <Grid item xs={12}>
          <Paper className={classes.paper}>{displayGetStarted()}</Paper>
        </Grid>
        {/* To be uncommented later */}
        <Grid item xs={6}>
  ...
  return (
    <div className={classes.footer}>
      ...
      {!(width < mobileBreakpoint) && <hr className={classes.bottomDivider} />}
      ...
      <div className={classes.copyrightsWrapper}>
        ...
        <Link
          href="/privacy-policy"
          className={[classes.copyrights, classes.privacyPolicyLink].join(' ')}
        >
